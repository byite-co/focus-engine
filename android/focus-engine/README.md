...
- `[24,24]` 도 `[30,30]` 도 없는 카메라(**fps unset**, HAL 기본값; E2 1장): 기대 처리 간격을 **가정하지 않는다**(30fps 도 아니다). 매 프레임 프리셋은 header 의 `face_process_period_ns`·`frame_gap_threshold_ns`·`frame_gap_threshold_ms` 가 null 이고 `nominal_fps` 는 0, 집계기는 워밍업 60초의 초당 CaptureResult 간격 중앙값의 중앙값을 **진단용 기대 간격**으로 배워(마지막 워밍업 버킷이 닫힐 때, `expected_interval_learned` 이벤트) 그 뒤 초부터 × 1.5 / 4.5 로 `gaps_over_threshold` 를 센다(워밍업 중에는 세지 않고 `gaps_over_80ms`·최대 갭만). 슬롯 프리셋(E·E15)은 슬롯 주기를 그대로 기대 간격으로 쓴다. 슬롯 규칙의 허용 오차는 기기가 주는 가장 빠른 range 상한의 프레임 간격 ÷ 2(가정이 아니라 하한; range 가 없으면 30fps 기준). 요약은 header 에 `fps unset(가변)`, 갭 임계 n/a(워밍업 실측 진단값 병기), 합격 판정 없음, 첫 줄 `비교 불가: fps unset(가변): 고정 AE range 요청 없음`. 세션이 60초 안에 끝나면 진단용 기대 간격도 없다.
