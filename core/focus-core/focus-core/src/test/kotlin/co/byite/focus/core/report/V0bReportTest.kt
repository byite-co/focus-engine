...
class V0bReportTest {
    private fun raw(sec: Long, label: String?, face: Double = 12.0, thermal: Int = 0, battery: Int? = 80, current: Int? = -400_000, voltage: Int? = 4000, interactive: Boolean = false, idle: Boolean = false, total: Double = face + 4, poseRequested: Int = 1, poseSamples: Int = 1) =
        V0bRawRecord(
    ...
    private val longRaws = (0L until 130L).map { sec ->
        raw(sec, if (sec < 70) "정면" else null, face = if (screenOn(sec)) 30.0 else 36.0, thermal = if (sec > 120) 2 else 1, interactive = screenOn(sec), current = if (screenOn(sec)) -500_000 else -400_000)
    }
    private val longLog = SessionLog(Synth.header.copy(capturePreset = "A", cameraResolution = "1280x720"), longRecords, sessionEnd = SessionEnd(Synth.mono(130), Synth.utc(130), SessionEndReason.USER), v0bRaw = longRaws)
    ...
    fun passMarkUsesTheOffRowAndThePresetDivisor() {
        ...
        val e = V0bReport.build(longLog.copy(header = longLog.header.copy(capturePreset = "E", frameProcessDivisor = 2, frameGapThresholdMs = 167)))
    ...
    @Test
    fun trendTableHasOneRowPerTenSeconds() {
        val t = V0bReport.build(longLog).trend
        ...
        val text = V0bReport.build(longLog).render()
