 ...
 * The logged `raw_state` / `final_state` are ignored as input and recomputed; lifecycle-gap intervals
 * in the log drive [StateFinalizer.onBackground] / [StateFinalizer.onForeground] so the flush and the
 * 10-minute rule are reproduced too.
 */
class ReplayRunner(
    ...
    fun run(log: SessionLog): ReplayResult {
        ...
        for (r in records) {
            ...
            outRecords.addAll(finalizer.push(r, engine.judge(r)))
        }
        ...
        if (end == null) {
            ...
            if (endAt != null) {
                val close = finalizer.endSession(endAt, logged?.tUtcMs, logged?.reason ?: SessionEndReason.UNKNOWN)
