 ...
 * The logged `raw_state` / `final_state` / `invalid_reason` / candidate fields are ignored as input
 * and recomputed (device events in `events` are kept); lifecycle-gap intervals in the log drive
...
class ReplayRunner(
    ...
    companion object {
        /** Events the device layer originates; every other event type is regenerated by the engine on replay. */
        val DEVICE_EVENTS: Set<EventType> = setOf(EventType.ZONE_ADDED)
    }
    ...
    fun run(log: SessionLog): ReplayResult {
        ...
        for (r in records) {
            ...
            val input = r.copy(events = r.events.filter { it.type in DEVICE_EVENTS })
