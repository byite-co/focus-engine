...
| `report` | `V0bReport`: V0-B/R4 세션 요약(계수 검증·cadence 불일치·stop_integrity_failed 줄, 프리셋 제목, 카메라 cadence 줄, 전체, 슬롯·진단 계수, 갭 임계 실제값, 화면 on/off 비교 행, 프리셋 합격(slot_miss_ratio; Hvar 는 판정 없음), 화면 상태 구간표, 10초 추이표, 마커별). 실시간 경로와 프로세스 종료 복원이 같은 함수를 쓴다 |
...
- `session_end`: `t_mono_ms, t_utc_ms, reason ∈ {USER, LIFECYCLE_GAP_TIMEOUT, PROCESS_DEATH_RECOVERED, UNKNOWN}`; 0.2.4 정상 종료 진단(옛 로그·복원은 null): `capture_results_before_start`, `capture_results_after_fence`, `capture_results_after_close`, `stop_integrity_failed`(after_close > 0). 정상 종료의 `t_mono_ms` 는 stop fence 다.
...
- header 의 0.2.3 추가(지시문 D): `capture_preset`(A, B, C, C2, D, E, G, 0.2.4 부터 E15, H12, H15, Hvar; 없으면 null), `frame_process_divisor`(1; E 는 2; 0.2.4 판독은 `face_process_period_ns` 우선), `frame_gap_threshold_ms`·`frame_long_gap_threshold_ms`(0.2.4 부터 ns 값의 반올림),
...
- header 의 0.2.4 추가(지시문 E): `camera_fps_request_lower`·`camera_fps_request_upper`(요청 AE range; 없으면 null; 고정 = lower == upper), `camera_fps_ranges_supported`("[7,15],[24,24],…"), `face_schedule`{every_frame, slot}, `face_process_period_ns`(기대 처리 간격; Hvar 는 상한 기준), `frame_gap_threshold_ns`·`frame_long_gap_threshold_ns`(실제 적용값 = 간격 × 1.5 / 4.5).
